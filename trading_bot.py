...
tp_percentage = 60 / 100  # TP +60%
sl_percentage = 50 / 100  # SL -50%
order_timeout = 60  # Hapus order jika tidak tereksekusi dalam 60 detik
...
def place_limit_order(order_type):
    try:
        ...
        # Tunggu order tereksekusi atau hapus jika terlalu lama
        ...
        while True:
            ...
            if time.time() - start_time > order_timeout:
                ...
                logging.info(f"Order {order_type} dibatalkan karena tidak tereksekusi dalam {order_timeout} detik.")
                send_telegram_message(f"⚠️ *Order {order_type} Dibatalkan* karena tidak tereksekusi dalam {order_timeout} detik.")
                return None
            time.sleep(5)

        # Ambil harga eksekusi terbaru
...
def place_tp_sl_orders(entry_price):
    try:
    ...
    while True:
        try:
            ...
            if spot_balance >= trade_amount:
                entry_price = place_limit_order("BUY")
                if entry_price:
                    place_tp_sl_orders(entry_price)
            else:
